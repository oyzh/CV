...
public:
	...
	void bestCandidate(){
		...
		//accum是四维的累加器矩阵，通过找它的最大值来得到相对应的参数。主要用大到哦id_max
		...
		//对应参考点（好像是将图片缩小了rangeXY倍计算，这里是还原，否则应该直接使用id_max中的值）
		...
		//所有的边界的关于参考点的位移都存在Rtable中有一份，这一步是把每一个边界点做旋转和尺度变化到检测的图像中。
		for (std::vector<std::vector<Vec2i> >::size_type ii = 0; ii < Rtable.size(); ++ii){
			for (std::vector<Vec2i>::size_type jj= 0; jj < Rtable[ii].size(); ++jj){
				int iimod = (ii+reff) % intervals;
			...
			for (int i=0; i<nc; ++i) {
			  //边界点是255,255,255。这里要计算边界点的梯度
				if ( data[i]==Vec3b(255,255,255)  )
				{
					...
					//要计算的三个量，即相对参考点的向量和梯度方向，注意梯度方向是指在原图中对应点的梯度方向
					rpt.dx = refPoint(0)-i;
					rpt.dy = refPoint(1)-j;
...
int main(){
  ...
  char  c='r';
	if (c == 't'){   
		...
		//Mat detect_img = imread("./files/Img_04.png", 1);
		Mat detect_img = imread("./files/xxx.bmp", 1);
